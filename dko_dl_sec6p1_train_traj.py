...
# -*- coding: utf-8 -*-
"""
DKO Sec.6.1 — Two-stage training (traj)

Stage 1:
  - Train encoder + decoder
  - Each batch: solve differentiable ridge-LS A* from latent adjacent pairs
  - Loss = lambda_lin * multi-step latent MSE (rollout latent) +
           lambda_pred * multi-step predictive NLL (rollout latent)

Stage 2 (decoder reconstruction only):
  - Freeze encoder
  - Do NOT use A at all
  - Train decoder only with teacher-forcing reconstruction NLL:
        NLL(theta_{t+k} | s_{t+k})

Logging:
  - GPU utilization via nvidia-smi at log time
  - CPU utilization via psutil (if available)

NLL subsampling:
  - nll_subsample (train): subsample K points for NLL to accelerate
  - nll_subsample_eval (eval): default full K unless set smaller
"""
...
def compute_stage1_losses(
    ...
    for k in range(1, H + 1):
        ...
        theta_k = subsample_theta(theta_seq[:, k, :], nll_subsample, rng=rng)
...
def compute_stage2_losses_recon(
    model: DLDKOModel,
    theta_seq: torch.Tensor,          # (B, H+1, K)
    gamma: float,
    nll_subsample: int,
    ...
    Stage 2:
      ...
      - No A at all
      - Decoder-only reconstruction NLL with teacher-forcing latent s_{t+k}
        Loss = weighted average over k=0..H of NLL(theta_{t+k} | s_{t+k})
    """
    B, Hp1, K = theta_seq.shape
    H = Hp1 - 1
    ...
    for k in range(0, H + 1):
        w = (gamma ** k) if gamma is not None else 1.0
        wsum += w
        theta_k = subsample_theta(theta_seq[:, k, :], nll_subsample, rng=rng)
        s_k = s[:, k, :]
        rec_k = model.decode_nll(theta_k, s_k, reduction="mean")
        rec_acc = rec_acc + w * rec_k

    rec = rec_acc / wsum
    loss = rec
    return loss, dict(rec=float(rec.item()))
...
def eval_epoch_stage2_recon(
    ...
    for theta_traj in loader:
        ...
        for k in range(0, H + 1):
            ...
            wsum += w
            theta_k = subsample_theta(theta_seq[:, k, :], nll_subsample_eval, rng=rng)
            rec_acc = rec_acc + w * model.decode_nll(theta_k, s[:, k, :], reduction="mean")

        rec = rec_acc / wsum
        loss = rec

        sums["loss"] += float(loss.item())
        sums["rec"] += float(rec.item())
        n_batches += 1
...
def main():
    ...
    ap.add_argument("--batch_measures", type=int, default=512)
    ap.add_argument("--lr", type=float, default=5e-4)
    ap.add_argument("--lr_stage2", type=float, default=None)
    ap.add_argument("--weight_decay", type=float, default=0.0)
    ap.add_argument("--grad_clip", type=float, default=1.0)
    ap.add_argument("--amp", action="store_true")
    ...
    # discount weights for multi-step aggregation
    ap.add_argument("--gamma", type=float, default=1.0)
    ...
    # stage2 recon horizon (optional override; default = eval_horizon)
    ap.add_argument("--stage2_horizon", type=int, default=None)

    # checkpoint selection (stage1)
    ap.add_argument("--select_metric", type=str, default="val_pred+lin",
                    choices=["val_loss", "val_pred", "val_lin", "val_pred+lin"])
    ...
    if "X_traj" in data:
        ...
        raise ValueError("Expected X_traj in npz for traj training.")
    ...
    run_cfg = dict(
        ...
        horizon=dict(
            start=int(args.horizon_start),
            max=int(horizon_max),
            warmup=int(args.horizon_warmup_epochs),
            eval_horizon=int(eval_horizon),
            eval_start_time=int(args.eval_start_time),
            stage2_horizon=(int(args.stage2_horizon) if args.stage2_horizon is not None else None),
        ),
        optim=dict(
            lr=float(args.lr),
            lr_stage2=float(args.lr_stage2 if args.lr_stage2 is not None else args.lr),
            batch_measures=int(args.batch_measures),
    ...
    if not os.path.exists(log_path):
        with open(log_path, "w") as f:
            f.write(
                "stage,epoch,H,train_loss,train_lin,train_pred,train_rec,val_loss,val_lin,val_pred,val_rec,select_score,"
    ...
    # -------------------------
    # Stage 2 (decoder recon only, no A)
    # -------------------------
    print(f"[stage2] epochs={args.epochs_stage2} | freeze encoder | decoder reconstruction only (no A)")
    ...
    # stage2 horizon: default eval_horizon
    H2 = int(min(args.stage2_horizon if args.stage2_horizon is not None else eval_horizon, L - 1))
    print(f"[stage2] recon horizon H2={H2}")

    best_stage2 = float("inf")
    best_epoch2 = 0

    # baseline eval
    val2 = eval_epoch_stage2_recon(
        model, val_loader, device,
        eval_horizon=H2,
        eval_start_time=args.eval_start_time,
        gamma=args.gamma,
    ...
    print(f"[stage2 init] val rec={val2['rec']:.6f}")
    ...
    for epoch2 in range(1, args.epochs_stage2 + 1):
        ...
        for theta_traj in train_loader:
            ...
            with torch.amp.autocast("cuda", enabled=amp_enabled):
                loss, mets = compute_stage2_losses_recon(
                    model, theta_seq,
                    gamma=args.gamma,
                    nll_subsample=args.nll_subsample,
                    rng=rng_torch,
        ...
        if do_log:
            val_m2 = eval_epoch_stage2_recon(
                model, val_loader, device,
                eval_horizon=H2,
                eval_start_time=args.eval_start_time,
                gamma=args.gamma,
            ...
            score2 = float(val_m2["rec"])  # selection: minimize recon NLL
            ...
            print(
                ...
                f"train rec={train2['rec']:.6f} | "
                f"val rec={val_m2['rec']:.6f} | "
                f"select(rec)={score2:.6f}{util_str}"
            )

            with open(log_path, "a") as f:
                f.write(
                    f"stage2,{epoch2},{H2},"
                    f"{train2['loss']},,,{train2['rec']},"
                    f"{val_m2['loss']},,,{val_m2['rec']},{score2},"
                    f"{cpu_u},{gpu_u},{gpu_mu},{mem_used},{mem_total}\n"
                )
            ...
            if score2 < best_stage2:
                ...
                torch.save(
                    dict(
                        ...
                        ridge=float(args.ridge),
                        val_metrics=val_m2,
                        best_rec=best_stage2,
                        stage2_horizon=int(H2),
                    ),
                    best_path,
                )
    ...
    print(f"[done] best stage2 rec={best_stage2:.6f} at ep2={best_epoch2} | ckpt: {best_path}")

    # Final report on val/test recon
    ckpt = torch.load(best_path, map_location=device)
    model.load_state_dict(ckpt["model_state"])

    val_final = eval_epoch_stage2_recon(
        model, val_loader, device,
        eval_horizon=H2,
        eval_start_time=args.eval_start_time,
        gamma=args.gamma,
    ...
    test_final = eval_epoch_stage2_recon(
    ...
    print(f"[best] val rec={val_final['rec']:.6f} | test rec={test_final['rec']:.6f}")

    with open(os.path.join(args.out_dir, "final_metrics.json"), "w") as f:
        json.dump(
            dict(
                stage1_best_score=float(best_score),
                stage2_best_rec=float(best_stage2),
                val=val_final,
                test=test_final,
